...
export const SongsScreen: React.FC<{ navigation: any; route: any }> = ({
  ...
  const [isDarkMode] = [usePlayerStore().isDarkMode];
  ...
  return (
    ...
    >
      ...
      {/* Inline Search Bar */}
      <View style={[styles.searchBar, { backgroundColor: isDarkMode ? Colors.darkGray : Colors.lightGray, borderColor: isDarkMode ? Colors.darkBorder : Colors.border }]}>
        <Feather name="search" size={18} color={Colors.gray} style={{ marginRight: 8 }} />
        <TextInput
          ref={searchInputRef}
          style={[styles.searchInput, { color: isDarkMode ? Colors.darkText : Colors.text }]}
          placeholder="Search songs..."
          placeholderTextColor={Colors.gray}
          value={searchQuery}
          onChangeText={(text) => {
            setSearchQuery(text);
            if (text.trim()) loadSongs(text);
          }}
          returnKeyType="search"
          onSubmitEditing={() => loadSongs(searchQuery)}
        />
      </View>
